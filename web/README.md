...
- 화면 구성(J5-019, 첫 실사용 뒤 정리): 상단 고정 바(이름·상태) → 이 앱으로 하는 일(3단계 안내) → 1단계 조사 설정(저장돼 있으면 접힘) → 2단계 물건 목록 → 3단계 지도(비어 있으면 안내를 지도 위에 겹침, 필지 경계 파일은 접힌 절) → 기록 폼(물건 선택 시) → 이 기기에 저장된 기록 → 내보내기 → 다른 기기로 옮길 때(접힘). 화면 문구는 사용자 용어(조사 이름, 연습용/실제, 물건 목록 파일, 필지 경계 파일)를 쓰고 내부 용어(study_id, synthetic, 시드, 파생본)는 힌트와 PC 문서에만 둔다. 스타일은 `styles.css` 의 색 토큰(라이트·다크)과 시스템 글꼴만 쓴다(외부 글꼴·CDN 없음). 버튼 리스너는 첫 렌더 전에 붙인다.

...
- `web/*.test.mjs`: 웹 모듈 단위 테스트(ZIP 작성기·내보내기는 Python zipfile·j5 inspect 를 오라클로 씀)와 브라우저 e2e(`app.e2e.test.mjs`, 헤드리스 Chromium + CDP, 다운로드한 ZIP 을 j5 inspect 로 검사. 브라우저 없으면 건너뜀). `node --test "tests/web/**/*.test.mjs"` (Node는 테스트 전용).

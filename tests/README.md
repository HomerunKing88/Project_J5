...
- `test_j5_017a_ops.py`: 운영 점검·휴면 재개. 할 일 목록과 통과 판정, 마지막 정상 상태 파일은 통과할 때만 갱신, 읽기 전용 열기가 마이그레이션을 적용하지 않음(구버전 정본은 대기 목록만 보고), 도구보다 새로운 정본 거절, 사진 누락·정본 없음, 새 PC 이전(빈 폴더 복구 → 점검 → 백업 재개), 반기 점검 기한, CLI.

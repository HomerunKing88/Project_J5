...
- `ops-check` 는 정본을 **읽기 전용**으로 열어(마이그레이션 없음) 한 번에 확인한다: 도구·정본 db_schema 버전과 대기 중인 마이그레이션 목록, 무결성·외래키 검사, 백업 상태(포인터·manifest 실제 확인, 마지막 백업 경과일), 파생본 상태, 사진 대사·수집 원본 대사, 마지막 활동(반영·수집 반영·파생본 게시·백업). 그 뒤 해야 할 일을 순서대로 낸다. 종료 코드 0 통과(할 일 없음) / 1 조치 필요 / 3 사용 오류. 아무것도 고치지 않는다.

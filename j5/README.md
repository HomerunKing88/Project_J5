...
- 물건이 없는 정본(예: `db init` 직후, 실거래만 반영한 상태)은 `project` 가 빈 파생본을 거절하므로 파생본 없음을 조치 항목이 아니라 `대상 없음` 표시로 둔다. 백업만 있으면 점검을 통과한다. 물건 목록을 넣으면(`load-seed`) 다음 점검부터 파생본이 조치 항목이 된다.

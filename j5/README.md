...
## 운영 점검·휴면 재개 (R6, J5-017A)

```text
python -m j5 db [--db 경로] ops-check [--data-home 경로] [--json]
```

- `ops-check` 는 정본을 **읽기 전용**으로 열어(마이그레이션 없음) 한 번에 확인한다: 도구·정본 db_schema 버전과 대기 중인 마이그레이션 목록, 무결성·외래키 검사, 백업 상태(포인터·manifest 확인 뒤 백업 폴더 전량 재검증: 파일 해시·DB 사본·사진 연결, 마지막 백업 경과일), 파생본 상태, 사진 대사·수집 원본 대사, 마지막 활동(반영·수집 반영·파생본 게시·백업). 그 뒤 해야 할 일을 순서대로 낸다. 종료 코드 0 통과(할 일 없음) / 1 조치 필요 / 3 사용 오류. 아무것도 고치지 않는다.
- 통과했을 때만 `J5_DATA_HOME/ops/last_known_good.json` 에 마지막 정상 상태를 쓴다: 앱·도구 버전, db_schema(도구·정본), package·projection·backup 스키마 버전, SQLite·Python 버전, dataset_version, 행 수, 백업·파생본 위치. 통과하지 못하면 이전 파일을 그대로 두고 "마지막 정상 상태" 로 보여 준다. 매 실행은 `logs/ops_check.log` 에 남는다.
- 반기 점검 기한: 마지막 통과 점검(없으면 마지막 백업)이 180일을 넘으면 `반기 점검 기한 초과` 로 표시한다. 표시일 뿐 조치 항목은 아니다.
- 물건이 없는 정본(예: `db init` 직후, 실거래만 반영한 상태)은 `project` 가 빈 파생본을 거절하므로 파생본 없음을 조치 항목이 아니라 `대상 없음` 표시로 둔다(파생본 포인터가 아예 없을 때만. 다른 정본의 파생본이나 손상된 포인터가 남아 있으면 조치 항목이다). 백업만 있으면 점검을 통과한다. 물건 목록을 넣으면(`load-seed`) 다음 점검부터 파생본이 조치 항목이 된다.
...
```text
python -m j5 view inspect <.j5view.zip 또는 게시 폴더> [--db 정본] [--json]
```

- 정본 없이 파생본을 검증(manifest 스키마·파일 해시·행수·참조·버전)하고 요약한다: projection_schema 버전(도구가 아는 버전인지), study_id·data_mode, 정본 버전·생성 시각, 범위 물건 수, 기록 수·종류별 수·관측일 범위, 사진 포함 여부, 파일 목록. ZIP 은 항목 이름(허용 목록·경로 탈출·백슬래시·제어문자)·암호화·심볼릭 링크·중복·항목 수·실제 읽은 바이트 한도를 확인하며 임시 폴더에 풀어 검사한다. 입력은 바꾸지 않는다. 종료 코드 0 통과 / 1 검증 실패 / 3 사용 오류.
- `--db` 를 주면 정본을 읽기 전용으로 열어 `최신 (정본 vN) / 구본 (정본 vN, 파생본 vM) / 정본보다 새로움 / 다른 정본의 파생본` 을 표시한다. 주지 않으면 최신 여부는 `미확인` 이다. 열람 결과는 게시·최신 여부·백업을 뜻하지 않는다.
- 새 폰·휴면 재개에서 오래된 `.j5view.zip` 을 쓰기 전에 이 명령으로 검증하고 구본 여부를 확인한 뒤, `assets.seed.json` 을 "시드 파일 불러오기" 로, `parcels.geojson` 을 "필지 파일 불러오기" 로 넣는다.

## 매입 준비 검토·진입조건 (R7, J5-018A)

```text
python -m j5 db [--db 경로] case-add <입력.json> [--json]
python -m j5 db [--db 경로] readiness --asset <asset_id> [--as-of YYYY-MM-DD] [--json]
python -m j5 db [--db 경로] readiness-approve --asset <asset_id> --on YYYY-MM-DD [--note ...]
python -m j5 db [--db 경로] readiness-withdraw --asset <asset_id> --on YYYY-MM-DD --reason ...
```

- `case-add`: 매입 준비 검토 기록(`schemas/acquisition_review.schema.json`, `kind: acquisition_review`, db_schema 13)을 불변 기록으로 넣는다. 전략, 범위(전체/지분/구분소유·필지·건물·구성 기준일), 가격(보증금 차감 전 계약 총액·보증금 총액·부가세·호가 확인 수준, 미확인은 null 과 사유), 참조 기록(목표 매수가·투자판단·자금안·개발안·규제 검토: 이 물건의 현재 기록), 매물 사건, 체크리스트 8항목(매입 범위·가격 / 금융 / 세무·법적 / 권리·임대차 / 건축·토지 / 사업·자금 / 계약 협상 / 최종 의사결정; 항목마다 상태·근거 문서·검토자·검토일·유효기한·재검토 조건·미해결 사항·해당 없음 사유). `verified` 는 검토일과 근거 문서 1건 이상 필수, `not_applicable` 은 사유 필수, `conditional`·`blocked` 는 미해결 사항 필수. 항목별 최소 재검토 조건은 도구가 정하고(`MANDATORY_TRIGGERS`) 입력은 조건을 더할 수만 있다. 정해지지 않은 유효기한은 null 로 두고 자동으로 채우지 않는다. 물건마다 현재 검토는 하나이며 새 검토는 `supersedes_id` 로 이전 검토를 가리킨다. 관심 단계는 바꾸지 않는다.
- `readiness`: 진입조건 판정. purchase_ready 인데 조건이 깨졌거나 승인 뒤 변경이 있으면 먼저 `[자동 해제]` 사유·판정 스냅샷과 함께 철회 이력을 남기고 관심 단계를 `detailed_review` 로 되돌린다(파생본 생성 `project` 도 게시 전에 같은 철회를 한다). 항목마다 `확인 유효 / 해당 없음 / 유효기한 경과 / 미완료 / 조건부 / 차단 / 변경 감지·재검토 필요`, 중요 미확인(계약 총액·보증금 명세·부가세·호가 확인, 자금안 없음·필요자기자본 미확정, 개발안 여유면적 미확정, 규제 효력 미확인·종료일 미확인, 참조 기록 없음), 검토 기록이 정본에 들어온 뒤의 변경(새 목표 매수가=가격, 새 자금안=대출, 새 개발안=전략, 새 규제 검토=규제, 물건 구성 변경=구성 토지, 변화 확인 관측=중요 실사 결과, 참조 기록 수정)을 그 변경을 재검토 조건으로 둔 항목에 붙인다. 준비 완료는 모든 항목이 확인 유효 또는 사유 있는 해당 없음이고 미확인·변경이 없을 때다. 종료 코드 0 준비 완료 / 1 준비 아님 또는 해제 필요.
- `readiness-approve`: 본인 승인. 준비 완료일 때만 `assets.tracking_status` 를 `purchase_ready` 로 바꾸고 `readiness_decisions` 에 불변 결정(이전·새 상태, 판정 스냅샷)을 남긴다. 이 상태는 계약 협상 준비 상태이며 대출 확약·허가·계약 적법성 보증이 아니다(ADR-07). `readiness-withdraw`: 사유와 함께 철회 결정을 남기고 관심 단계를 `detailed_review` 로 되돌린다. 승인 이력은 지워지지 않는다. 재전환은 새 검토 기록(변경 반영) + 승인이다.
- 계약·송금·중개사 연락·전문가 예약은 자동화하지 않는다.

## 계약 직전·잔금 직전 재확인과 비교자료 내보내기 (R7, J5-018B)

```text
python -m j5 db [--db 경로] case-recheck <입력.json> [--json]
python -m j5 db [--db 경로] case-export [--asset <asset_id> ...] [--data-home 경로] [--out 상위폴더] [--as-of YYYY-MM-DD] [--json]
```
